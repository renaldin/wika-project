        ...
        <div class="card">
            ...
            <div class="card-body px-4" style="margin-bottom: -50px;">
                <div class="row">
                    ...
                    <div class="col-lg-4 mb-3">
                        <form action="{{ route('daftar-laporan-pajak') }}" method="GET">
                    ...
                    @if ($user->role == 'Tim Proyek')
                        <div class="col-lg-8 mb-1">
                            <a href="/tambah-laporan-pajak" class="btn btn-primary mb-4">Tambah</a>
                        </div>
                    @endif
                    @if (session('success'))
                        <div class="col-lg-12">
            ...
            <div class="card-body px-0">
                ...
                <table id="user-list-table" class="table table-striped" role="grid" data-toggle="data-table">
                    ...
                    <tbody>
                        <?php $no = 1;?>
                        @foreach ($daftarLaporanPajak as $item)
                            <tr>
                                <td>{{$no++}}</td>
                                <td>{{$item->proyek?->nama_proyek}}</td>
                                <td>{{$item->periode}}</td>
                                {{-- <td>{{$item->verifikasi_pajak}}</td> --}}
                                <td>
                                    <div class="flex align-items-center list-user-action">
                                        @if ($user->role == 'Tim Proyek')
                                            @if (!$item->id_verifikator)
                                                <button type="button" class="btn btn-sm btn-icon btn-danger btn-delete" data-toggle="tooltip" data-placement="top" title="Hapus" data-original-title="Hapus" data-href="/hapus-laporan-pajak/{{$item->id}}" data-content="Apakah Anda yakin akan hapus data ini?">
                                            ...
                                            @endif
                                        @endif
                                        <a href="/detail-laporan-pajak/{{$item->id}}" class="btn btn-sm btn-icon btn-primary" data-toggle="tooltip"  data-placement="top" title="Detail" data-original-title="Detail">
                                            <span class="btn-inner">
