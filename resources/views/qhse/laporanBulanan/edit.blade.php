...
<div class="row">
    <div class="col-sm-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                <div class="header-title">
                    <h4 class="card-title">Detail Laporan HSE</h4>
                </div>
            </div>
                    ...
                    <table id="user-list-table" class="table table-striped" role="grid">
                        <thead>
                            <tr class="ligth">
                                ...
                                <th>Jumlah File</th>
                                <th>Status</th>
                                @if($laporanBulananDetail->contains('status', 2)) <!-- Cek apakah ada status = 2 -->
                                    <th>Komentar</th>
                                @endif
                        ...
                        <tbody>
                            @foreach ($laporanBulananDetail as $index => $item)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $item->dokumen?->dokumen }}</td>
                                    <td>{{ $item->laporan_bulanan_sub_details_count ?? 0 }}</td>
                                    <td>
                                        @if($item->status == 0)
                                            ...
                                            @if ($user->role == 'Head Office')
                                                @if($item->status == 0)
                                                    ...
                                                    <form id="form-verifikasi-{{ $item->id }}" 
                                                          action="{{ url('/verifikasi-detail-laporan-bulanan/'.$item->id) }}" 
                                                          method="POST" 
                                                ...
                                                @else
                                                    ...
                                                    <form id="form-buka-verifikasi-{{ $item->id }}" 
                                                          action="{{ url('/bukaverifikasi-detail-laporan-bulanan/'.$item->id) }}" 
                                                          method="POST" 
                                                          style="display:inline;">
                                                        @csrf
                                                ...
                                                @endif
                                            @endif
                                            <a href="{{ url('/sub-detail-laporan-bulanan/' . $item->id) }}" class="btn btn-sm btn-icon btn-info" data-toggle="tooltip" title="Edit">
                                                <span class="btn-inner">
        ...
        <div class="modal-content">
            <form method="POST" id="rejectForm">
                ...
                <div class="modal-header">
                    ...
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close" onclick="$('#rejectModal').modal('hide');">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal" onclick="$('#rejectModal').modal('hide');">Batal</button>
...
function openRejectModal(itemId) {
    ...
    document.getElementById('rejectForm').action = '/tolak-verifikasi-detail-laporan-bulanan/' + itemId;
