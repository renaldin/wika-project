                    ...
                    <table id="user-list-table" class="table table-striped" role="grid">
                        <thead>
                            <tr class="ligth">
                                ...
                                <th>Jumlah File</th>
                                <th>Status</th>
                                @if($laporanQaDetail->contains('status', 2)) <!-- Cek apakah ada status = 2 -->
                                    <th>Komentar</th>
                                @endif
                        ...
                        <tbody>
                            @foreach ($laporanQaDetail as $index => $item)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                    <td>{{ $item->dokumen?->dokumen }}</td>
                                    <td>{{ $item->laporan_qa_sub_details_count ?? 0 }}</td>
                                    <td>
                                        @if($item->status == 0)
                                            ...
                                            @if ($user->role == 'Head Office')
                                                @if($item->status == 0)
                                                    ...
                                                    <form id="form-verifikasi-{{ $item->id }}" 
                                                          action="{{ url('/verifikasi-detail-laporan-qa/'.$item->id) }}" 
                                                          method="POST" 
                                                ...
                                                @else
                                                    ...
                                                    <form id="form-buka-verifikasi-{{ $item->id }}" 
                                                          action="{{ url('/bukaverifikasi-detail-laporan-qa/'.$item->id) }}" 
                                                          method="POST" 
                                                          style="display:inline;">
                                                        @csrf
                                                ...
                                                @endif
                                            @endif
                                            <a href="{{ url('/sub-detail-laporan-qa/' . $item->id) }}" class="btn btn-sm btn-icon btn-info" data-toggle="tooltip" title="Edit">
                                                <span class="btn-inner">
...
function openRejectModal(itemId) {
    ...
    document.getElementById('rejectForm').action = '/tolak-verifikasi-detail-laporan-qa/' + itemId;
