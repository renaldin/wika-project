...
class RkpMankon extends Controller
{

    private $ModelProyek, $ModelRkpMankon, $ModelDetailTimProyek, $ModelUser, $public_path, $public_path_hasil;
    ...
    public function index()
    {
        ...
        $data = [
            ...
            'daftarRkpMankon'                 => ModelRkpMankon::with('userRespon', 'proyek')->where('is_respon', 1)->orderBy('id_rkp_mankon', 'DESC')->get(),
        ...
        return view('rkp_mankon.index', $data);
    }
    ...
    public function updateDokumen()
    {
        ...
        $data = [
            ...
            'daftarRkpMankon'                 => $this->ModelRkpMankon->data(),
        ...
        return view('rkp_mankon.updateDokumen', $data);
    }
    ...
    public function tambah()
    {
        ...
        return view('rkp_mankon.form', $data);
    }
    ...
    public function prosesTambah()
    {
        ...
        $this->ModelRkpMankon->tambah($data);
        ...
        return redirect()->route('update-rkp-mankon')->with('success', 'Anda telah mengirimkan data RKP ke Tim Proyek! Data akan muncul di tabel update jika Tim Proyek telah upload file dokumen. ');
    }
    ...
    public function prosesHapus($id_proyek)
    {
        ...
        $proyekUser = ProyekUsers::where('id_proyek', $id_proyek)->get();
        foreach ($proyekUser as $item) {
            $item->delete();
        }
        ...
        $log            = new ModelLog();
        $log->id_user   = Session()->get('id_user');
        $log->activity  = 'Menghapus Data RKP MANKON';
        $log->feature   = 'RKP';
        $log->save();

        return redirect()->route('update-rkp-mankon')->with('success', 'Data RKP berhasil dihapus !');
    }
    public function terima()
    {
        ...
        return view('rkp_mankon.terima', $data);
    }

    public function edit($id_rkp_mankon)
    {
        if (!Session()->get('role')) {
            return redirect()->route('login');
        }
        ...
        $data = [
            ...
            'daftarProyek'      => $dataProyekByUser,
            'detail'            => $this->ModelRkpMankon->detail($id_rkp_mankon),
            'user'              => $this->ModelUser->detail(Session()->get('id_user')),
            'form'              => 'Edit'
        ];
    ...
    public function prosesEdit($id_rkp_mankon)
    {
        if (!Session()->get('role')) {
            return redirect()->route('login');
        }
        ...
        return redirect()->route('update-rkp-mankon')->with('success', 'Data berhasil diupdate!');
    }

    public function prosesUpdateDokumen($id_rkp_mankon)
    {
        if (!Session()->get('role')) {
            return redirect()->route('login');
        }
    ...
    public function update()
    {
        ...
        return view('rkp_mankon.update', $data);
    }
    ...
    public function downloadFile($id_rkp_mankon)
    {
        if (!Session()->get('role')) {
            return redirect()->route('login');
        }

        $data = $this->ModelRkpMankon->detail($id_rkp_mankon);

        $fileName = $data->upload_file;
        return response()->download(public_path($this->public_path) . '/' . $fileName);
    }
    
    public function downloadFileHasil($id_rkp_mankon)
    {
        if (!Session()->get('role')) {
            return redirect()->route('login');
        }
    ...
    public function downloadPdf(Request $request)
    {
        ...
        $rkp = ModelRkpMankon::with('userRespon', 'proyek')
            ->whereBetween('tanggal_rkp', [$request->start_date, $request->end_date])
            ->limit(300)
            ->get();

        $pdf = Pdf::loadview('rkp_mankon.downloadPdf', [
            'namaFile'          => $namaFile,
            'daftarRkpMankon'         => $rkp
